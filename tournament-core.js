...
export function defaultState() {
  return {
    ...
    prizeCount: 4,
    prizes: [
      { place: 1, reward: 'A definir pelo administrador' },
      { place: 2, reward: 'A definir pelo administrador' },
      { place: 3, reward: 'A definir pelo administrador' },
      { place: 4, reward: 'A definir pelo administrador' },
      { place: 5, reward: 'A definir pelo administrador' }
    ],
    updatedAt: null
  };
}
...
export function normalizeState(value) {
  ...
  const rawCount = Number(value.prizeCount ?? defaults.prizeCount);
  const prizeCount = Math.min(5, Math.max(1, Number.isFinite(rawCount) ? Math.trunc(rawCount) : defaults.prizeCount));
  const sourcePrizes = Array.isArray(value.prizes) ? value.prizes : defaults.prizes;
  const prizes = defaults.prizes.map((fallback, index) => {
    const item = sourcePrizes[index] || {};
    return { place: index + 1, reward: String(item.reward || fallback.reward).trim().slice(0, 100) };
  });
  return {
